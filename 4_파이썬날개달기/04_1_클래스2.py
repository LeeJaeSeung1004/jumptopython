...
# 그렇다면 a 객체의 first는 3으로 변할까? 아니면 기존 값 4를 유지할까? 다음과 같이 그 결과를 확인해 보자.
...
#이 연산이 가능하도록 다음과 같이 FourCal클래스를 만들어 보자
...
# a = FourCal()을 수행할 때 생성자 __init__이 호출되어 오류가 발생했다.
...
# >>> print(a.lastname)
# 최
# 이렇게 하면 Family 클래스의 lastname이 바뀌는 것이 아니라 a 객체에 lastname이라는 객체변수가 새롭게 생성된다. 즉, 객체변수는 클래스 변수와 동일한 이름으로 생성할 수 있다. 물론 a.lastname은 이제부터 Family 클래스의 lastname이 아닌 객체 a의 객체변수 lastname을 가리킨다.
